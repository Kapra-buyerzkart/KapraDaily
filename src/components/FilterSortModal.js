...
const FilterSortModal = ({ visible, onClose, onApply, initialSort, initialMin, initialMax }) => {
    ...
    const [leftPos, setLeftPos] = useState(new Animated.Value((initialMin / maxVal) * sliderWidth));
    const [rightPos, setRightPos] = useState(new Animated.Value((initialMax / maxVal) * sliderWidth));
    ...
    const panResponderLeft = useRef(
        PanResponder.create({
            ...
            onPanResponderMove: (evt, gestureState) => {
                const newPos = Math.max(0, Math.min(gestureState.moveX - wp('12.5%'), rightPos._value - 20));
    ...
    const panResponderRight = useRef(
        PanResponder.create({
            onStartShouldSetPanResponder: () => true,
            onPanResponderMove: (evt, gestureState) => {
                const newPos = Math.min(sliderWidth, Math.max(leftPos._value + 20, gestureState.moveX - wp('12.5%')));
                    ...
                    <View style={styles.body}>
                        ...
                        <View style={styles.sortContainer}>
                            {SORT_OPTIONS.map((option) => (
                                <TouchableOpacity
                                    key={option.value}
                                    style={[styles.sortOption, selectedSort === option.value && styles.selectedSortOption]}
                                    onPress={() => setSelectedSort(option.value)}
                                >
                                    <Text style={[styles.sortLabel, selectedSort === option.value && styles.selectedSortLabel]}>
                                        {option.label}
                                    </Text>
                                </TouchableOpacity>
                            ))}
                        ...
                        <Text style={[styles.sectionTitle, { marginTop: hp('3%') }]}>Price Range</Text>
                        <View style={styles.priceDisplay}>
                            <Text style={styles.priceLabel}>₹{minPrice} - ₹{maxPrice}+</Text>
                        </View>

                        <View style={styles.sliderContainer}>
                            <View style={styles.sliderTrack} />
                            <Animated.View
                                style={[styles.sliderActiveTrack, {
                                    left: leftPos,
                            ...
                            <Animated.View
                                ...
                                style={[styles.sliderThumb, { left: Animated.subtract(leftPos, 10) }]}
                            />
                            <Animated.View
                                {...panResponderRight.panHandlers}
                                style={[styles.sliderThumb, { left: Animated.subtract(rightPos, 10) }]}
                            />
                        </View>
