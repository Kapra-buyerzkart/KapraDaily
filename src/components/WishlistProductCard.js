...
const WishlistProductCard = ({ item, onRemove, onAddToCart, onPress }) => {
  ...
  const discountPercentage =
    item.unitPrice && item.specialPrice
      ? Math.round(
          ((item.unitPrice - item.specialPrice) / item.unitPrice) * 100,
        )
      : 0;

  const isOutOfStock =
    item.isAvailable === false ||
    (item.stockQty !== undefined && Number(item.stockQty) === 0);
      ...
      <View style={styles.productCardViewTwo}>
        ...
        <Text
          style={[
            ...
            { color: isOutOfStock ? '#FF0000' : '#0CA201' },
          ]}
        >
          {isOutOfStock ? 'OUT OF STOCK' : 'In Stock'}
        </Text>
        {discountPercentage > 0 && (
          <Text style={styles.offerText}>{discountPercentage}% OFF</Text>
        )}
      </View>
      ...
      <View style={styles.productCardViewFour}>
        {quantity > 0 ? (
          ...
          <TouchableOpacity
            style={[
              styles.plusIconView,
              isOutOfStock && { backgroundColor: '#CCCCCC' },
            ]}
