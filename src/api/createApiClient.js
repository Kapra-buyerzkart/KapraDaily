...
const createApiClient = ({
  ...
  let logoutHandler = () => {
    logger.warn(`⚠️ [${label}]: Logout handler called but not registered.`);
  };
  ...
  const errorHandler = error => {
    ...
    if (
      ...
      logger.log(
        `[${label}]: Database identity conflict detected (error), triggering logout.`,
      );
      if (!isLoggingOut) {
        isLoggingOut = true;
        logoutHandler(true);
  ...
  const resetNetworkState = () => {
    ...
    isLoggingOut = false;
    failedQueue = [];
    logger.log(` [${label}]: Network state reset.`);
  };
  ...
  axiosInstance.interceptors.response.use(
    response => {
      ...
      if (data && data.success === false && data.message) {
        ...
        if (
          ...
          logger.log(
            `[${label}]: Database identity conflict detected (success branch), triggering logout.`,
          );
          if (!isLoggingOut) {
            isLoggingOut = true;
            logoutHandler(true);
    ...
    async error => {
      ...
      if (
        ...
        try {
          logger.log(`🔄 [${label}]: Attempting token refresh...`);
          const res = await axios.post(`${baseUrl}auth/refreshtoken`, {
            refreshToken: refreshToken,
          });

          logger.log(`🔄 [${label}]: Refresh response received:`, res.status);

          const apiData = res.data?.data || res.data?.Data || res.data;
          const newAccessToken = apiData?.accessToken || apiData?.access_token;
          const newRefreshToken =
            apiData?.refreshToken || apiData?.refresh_token;

          if (newAccessToken) {
            logger.log(
              `✅ [${label}]: Token refresh successful, storing new tokens`,
            );
