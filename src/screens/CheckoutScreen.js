...
const CheckoutScreen = () => {
    ...
    useEffect(() => {
        ...
        if (currentPincodeAreaId) {
            ...
            logger.log('🔄 [CHECKOUT] Syncing summary for area:', currentPincodeAreaId, 'Mode:', apiDeliveryMode);
    ...
    useEffect(() => {
        const fetchPaymentModes = async () => {
            try {
                ...
                logger.log('💳 [CHECKOUT] Fetched Payment Modes:', JSON.stringify(response?.data, null, 2));
                ...
                if (response?.success && response?.data) {
                    ...
                    if (!hasOnline) {
                        logger.log('💳 [CHECKOUT] Injecting Online mode for testing purposes.');
                        modes.push({
    ...
    const handleConfirmOrder = async () => {
        ...
        try {
            ...
            if (resumeOrderId) {
                ...
                logger.log('🔄 [CHECKOUT] Resuming payment for existing order:', resumeOrderId);
            ...
            logger.log('📦 [CHECKOUT] Creating Order Payload:', JSON.stringify(createPayload, null, 2));
            const createResponse = await createOrderApi(createPayload);
            logger.log('📦 [CHECKOUT] Create Response:', JSON.stringify(createResponse, null, 2));
    ...
    const handlePaymentFlow = async (orderId, orderNumber, passedRzpOrderId, passedAmount, passedKeyId) => {
        try {
            ...
            if (!razorpayOrderId || !amount || !keyId) {
                ...
                logger.log('💳 [RAZORPAY] Create Response:', JSON.stringify(rzpCreateResponse, null, 2));
            ...
            setTimeout(async () => {
                try {
                    ...
                    const attemptVerification = async () => {
                        try {
                            logger.log(`🔍 [RAZORPAY] Verification Attempt ${retryCount + 1}...`);
                            const response = await verifyRazorpayPaymentApi(verifyPayload);
                            return response;
                        } catch (e) {
                            logger.error(`⚠️ [RAZORPAY] Verification Attempt ${retryCount + 1} Error:`, e);
                            return null;
                        }
                    };
                    ...
                    while (
                        ...
                        retryCount++;
                        logger.log(`🔄 [RAZORPAY] Retrying verification (Count: ${retryCount}) in 3s...`);
    ...
    const handleVerificationFailure = async (orderId, orderData, sdkResponse, keyId, amount) => {
        logger.log('⚠️ [RAZORPAY] Verification failed or timed out. Checking order details...');
        setStatusType('info');
        ...
        setTimeout(async () => {
            try {
                ...
                logger.log('🔍 [RAZORPAY] Order Details Check:', response?.data?.orderStatusKey);
                ...
                logger.error('Error fetching order details for verification fallback:', err);
                showLoader(false);
                setStatusType('error');
