...
const CheckoutScreen = () => {
  ...
  useEffect(() => {
    ...
    if (currentPincodeAreaId) {
      ...
      logger.log(
        '🔄 [CHECKOUT] Syncing summary for area:',
  ...
  const frontendBillCalculations = React.useMemo(() => {
    ...
    const totalSavings =
      savings + (deliveryCharge === 0 && itemTotal >= 500 ? 5 : 0);
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      ...
      const createPayload = {
        ...
        pincodeAreaId:
          pincodeAreaId ||
          currentSelectedAddress?.pincodeAreaId ||
      ...
      if (createResponse?.success && createResponse?.data?.orderId) {
        ...
        logger.log('🔄 [CHECKOUT] Conflict detected, refreshing cart...');
        await refreshCart();
        showLoader(false);
        setStatusType('error');
        setStatusTitle('Cart Updated');
        setStatusMessage(
          'Your cart was updated during checkout. Please try confirming your order again.',
        );
        setStatusModalVisible(true);
      } else {
        showLoader(false);
  ...
  const handlePaymentFlow = async (
    ...
    try {
      ...
      if (!razorpayOrderId || !amount || !keyId) {
        logger.log('💳 [RAZORPAY] Initiating Payment Flow for Order:', orderId);
        ...
        if (rzpCreateResponse?.success && rzpCreateResponse?.data) {
          ...
          setStatusTitle('Error');
          setStatusMessage(
            rzpCreateResponse?.message || 'Failed to initiate payment',
          );
          setStatusModalVisible(true);
          return;
        }
      ...
      setTimeout(async () => {
        try {
          ...
          const attemptVerification = async () => {
            try {
              logger.log(
                `🔍 [RAZORPAY] Verification Attempt ${retryCount + 1}...`,
              );
              const response = await verifyRazorpayPaymentApi(verifyPayload);
              return response;
            } catch (e) {
              logger.error(
                `⚠️ [RAZORPAY] Verification Attempt ${retryCount + 1} Error:`,
                e,
              );
              return null;
            }
          ...
          while (
            ...
            retryCount++;
            logger.log(
              `🔄 [RAZORPAY] Retrying verification (Count: ${retryCount}) in 3s...`,
            );
  ...
  const handleVerificationFailure = async (
    ...
    logger.log(
      '⚠️ [RAZORPAY] Verification failed or timed out. Checking order details...',
    );
    setStatusType('info');
    setStatusTitle('Verifying Payment...');
    setStatusMessage(
      'Your payment verification is taking longer than expected. Please wait...',
    );
    setStatusModalVisible(true);
    ...
    setTimeout(async () => {
      try {
        ...
        logger.log(
          '🔍 [RAZORPAY] Order Details Check:',
          response?.data?.orderStatusKey,
        );

        if (
          response?.success &&
          response?.data?.orderStatusKey?.toLowerCase() === 'placed'
        ) {
          setStatusModalVisible(false);
          await finalizeOrder(orderData);
        } else if (
          response?.data?.orderStatusKey?.toLowerCase() === 'pending'
        ) {
          setStatusModalVisible(false);
          showLoader(false);
          navigation.navigate('OrderPendingScreen', {
        ...
        logger.error(
          'Error fetching order details for verification fallback:',
          err,
        );
        showLoader(false);
        setStatusType('error');
  ...
  const finalizeOrder = async orderData => {
    ...
    navigation.navigate('OrderSuccessScreen', {
      ...
      deliverySlot: chosenSlot
        ...
        : null,
      address:
        currentSelectedAddress?.address || selectedAddress?.address || '',
    });
  };
          ...
          <View style={styles.card}>
            <View style={{ flex: 1 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                {(() => {
                  const type =
                    currentSelectedAddress?.type?.toLowerCase() || '';
                ...
                <Text style={[styles.addressType, { marginTop: 5 }]}>
                  {currentSelectedAddress?.type || 'Home'}
                </Text>
              </View>
              <View style={styles.addressDivider} />
              <Text style={styles.addressText}>
                {currentSelectedAddress?.address || 'No address selected'}
              </Text>
            ...
            >
              ...
              <View style={styles.radioContent}>
                <Text
                  style={[
                    styles.radioTitle,
                    deliveryType === 'express' && { color: '#F25000' },
                  ]}
                >
                  Express Delivery
                </Text>
            ...
            >
              ...
              <View style={styles.radioContent}>
                <Text
                  style={[
                    styles.radioTitle,
                    deliveryType === 'slot' && { color: '#F25000' },
                  ]}
                >
                  Slotted Delivery
                </Text>
                ...
                <View key={item.id || index} style={styles.itemRow}>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.itemName} numberOfLines={1}>
                      {item.prName || item.productName || item.name}
                    </Text>
                  ...
                  <Text style={styles.itemQty}>
                    x{item.quantity || item.addedQty}
                  </Text>
                  <Text style={styles.itemTotalText}>
                    ₹{(price * (item.quantity || item.addedQty)).toFixed(2)}
                  </Text>
                </View>
              );
            })}
        ...
        >
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            <Text style={styles.totalAmount}>
              ₹{billCalculations.toPay?.toFixed(2) || '0.00'}
            </Text>
            <Image
