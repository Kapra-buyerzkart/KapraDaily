...
function PriorityChip({ option, selected, onPress }) {
  ...
  const chipStyle = useAnimatedStyle(() => ({
    backgroundColor: withTiming(
      selected ? option.soft : LUXURY_COLORS.well,
      CHIP_FADE,
    ),
    borderColor: withTiming(
      selected ? (option.border || option.tint) : LUXURY_COLORS.border,
      CHIP_FADE,
    ),
    ...
    >
      <Animated.View style={[styles.priorityChip, chipStyle]}>
        <MaterialCommunityIcons
          name={option.icon}
          size={ICON.chip}
          color={selected ? option.tint : LUXURY_COLORS.textMuted}
        />
        <Text
          style={[
            styles.priorityChipText,
            { color: selected ? option.tint : LUXURY_COLORS.textSecondary },
