...
const HomeScreen = () => {
  ...
  const { profile, loadProfile, loadProfileTwo, logout } =
  ...
  const handlePopupPress = () => {
    ...
    if (popupData.popupId) {
      postPopupSeenApi(popupData.popupId).catch(err =>
        console.error('Popup seen API error on press:', err),
      );
    }
    ...
    if (numericProductId > 0) {
      console.log(
        '🔗 [POPUP] Navigating to product (productId is ' +
          numericProductId +
          ')',
      );
    ...
    if (finalLink) {
      ...
      Linking.openURL(finalLink).catch(err =>
        console.error("Couldn't load external page", err),
      );
    } else {
  ...
  const onRefresh = useCallback(async () => {
    setRefreshing(true);
    try {
      await Promise.all([fetchDashboardData(), refreshHomeData()]);
    } catch (error) {
    ...
    <View style={styles.mainContainer}>
      ...
      <HomePopupModal
        ...
        onClose={() => {
          ...
          if (popupData?.popupId) {
            postPopupSeenApi(popupData.popupId).catch(err =>
              console.error('Popup seen API error:', err),
            );
          }
        }}
      ...
      {topSectionBanner && topSectionBanner.length > 0 ? (
        <TouchableOpacity
          activeOpacity={0.9}
          onPress={() => handleBannerPress(topSectionBanner[0])}
        >
          <ImageBackground
                    ...
                    {secondProductBlock?.image !== null &&
                      secondProductBlock?.image !== undefined && (
                        <Image
                          source={
                            secondProductBlockTitleImage
                              ? secondProductBlockTitleImage.uri
                              : {
                                  uri: `${CONFIG.image_base_url}${
                                    secondProductBlock?.Image ||
                                    secondProductBlock?.image
                                  }`,
                                }
                          }
                          ...
                          >
                            {discoveryCategories.map((item, index) => (
                              <TouchableOpacity
                                ...
                                style={styles.item}
                                onPress={() =>
                                  handleDiscoveryCategoryPress(item)
                                }
                              >
                                <View
                                  style={[
                                    styles.categoryItemContainer,
                                    selectedDiscoveryCategory?.catId ===
                                      item.catId &&
                                      styles.categoryItemContainerActive,
                                  ]}
                                >
                                  <Image
                                    source={{
